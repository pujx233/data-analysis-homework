...
for i in load_dict:  # i为学生id j为case_id
    ...
    dir[i]['线性表平均得分'] = 0
    dir[i]['线性表提交次数'] =0
    dir[i]['score'] = [dir[i]['字符串平均得分'],dir[i]['图结构平均得分'],dir[i]['树结构平均得分'],dir[i]['数字操作平均得分'],dir[i]['排序算法平均得分'],dir[i]['查找算法平均得分'],dir[i]['数组平均得分'],dir[i]['线性表平均得分']]

for i in load_dict:  # i为学生id j为case_id
    for j in load_dict[i]['cases']:
        if j["case_type"]=='字符串':
            dir[i]['字符串提交次数'] += 1
            dir[i]['字符串平均得分'] +=j['final_score']
        if j['case_type']=='图结构':
            dir[i]['图结构提交次数'] += 1
            dir[i]['图结构平均得分'] += j['final_score']
        if j['case_type']=='树结构':
            dir[i]['树结构提交次数'] += 1
            dir[i]['树结构平均得分'] += j['final_score']
        if j['case_type']=='数字操作':
            dir[i]['数字操作提交次数'] += 1
            dir[i]['数字操作平均得分'] += j['final_score']
        if j['case_type'] == '排序算法':
            dir[i]['排序算法提交次数'] += 1
            dir[i]['排序算法平均得分'] += j['final_score']
        if j['case_type'] == '查找算法':
            dir[i]['查找算法提交次数'] += 1
            dir[i]['查找算法平均得分'] += j['final_score']
        if j['case_type'] == '数组':
            dir[i]['数组提交次数'] += 1
            dir[i]['数组平均得分'] += j['final_score']
        if j['case_type'] == '线性表':
            dir[i]['线性表提交次数'] += 1
            dir[i]['线性表平均得分'] += j['final_score']
...
sorted(dir)
print(len(dir))
with open("../data/users_analysis_source.json", 'w', encoding='utf-8') as ans:
